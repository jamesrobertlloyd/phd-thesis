...
This work was completed before the work described in chapter~\ref{ch:construction} hence the construction of kernels was not automated.
The techniques described in this chapter are relatively simple, but they were sufficient to be ranked \nth{2} in the competition.
At the end of this chapter\fTBD{ref} I will discuss how the kernels could have been automatically constructed which will identify some short comings in the literature on approximation inference methods for Gaussian processes.
...
Perhaps not too surprisingly this resulted in many participants overfitting heavily to this particular split of the data; the leaderboard changed substantially when final scores were evaluated on the remaining 75\% of the data; the private scores.
...
Many readers can safely ignore references to code and replication.
...
In practice this was usually numerically equivalent to selecting the model / temperature station with the highest marginal likelihood.
...
\section{Constructing the composite kernels automatically}
...
Presented with this failure one might consider instead using a contiguous subset of subset closest to the times at which prediction will be performed.
...
There is also insufficient data to infer that whilst the shape of periodicity changes smoothly over time there is an historical average that it should return to.
