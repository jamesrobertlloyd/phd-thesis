...
The assumption of exchangeability stated in~\eqref{eq:exchangeability} states that the order of the data does not matter.
...
\fTBD{That said, their most recent work is very scaling.}
...
Since we focus on undirected graphical data, we require the symmetry condition $\larray_{ij} = \larray_{ji}$. This can be achieved by constructing the kernel function in the following way\fTBD{Cite Duvenaud thesis}
\begin{eqnarray}
...
Hoff's eigenmodel \citep{Hoff2007a-ja}, and the GPLVM \citep{Lawrence2005-cn}. The models are chosen for comparability, since they all embed nodes into a Euclidean latent space.
...
The degree distribution of the left and right blocks is also of interest; the variation is larger than expected by a block model (assuming that the probability of a link between different blocks is constant).\fTBD{Try to find out what these functional groups are - one block is enzymes IIRC}
