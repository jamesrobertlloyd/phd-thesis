...
This chapter is based on joint work with Peter Orbanz, Zoubin Ghahramani and Daniel Roy \TBD{cite}.
...
We consider statistical models of infinite 2-arrays $(\darray_{ij})_{i,j\in\Nats}$\fTBD{make consistent with other set notation}, where entries $\darray_{ij}$ take values in a space $\dataspace$. 
...
To specify probabilistic models for graph- or array-valued data, we need a suitable counterpart to de Finetti's theorem applicable when the infinite random sequences in \eqref{eq:exchangeability} are substituted by infinite random arrays $\darray=(\darray_{ij})_{i,j\in\Nats}$.
...
To define a probabilistic model for matrix\fTBD{Change me to array throughout}- or graph-valued data, we start with Theorem \ref{theorem:ah}: A distribution on exchangeable matrices
