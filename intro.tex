...
All chapters of this thesis apart from chapters~\ref{ch:criticism}~and~\ref{ch:gefcom} take an entirely (or at least approximately) Bayesian approach to statistical inference.
...
\end{enumerate}
The calculus that one can derive from these axioms alone is isomorphic to probability theory; plausabilities satisfy the sum and product rules that are traditionally used as the axioms of probability theory\fTBD{cite e.g. Kolmogorov}.
...
Rather than repeat them, I will state the key facts required to understand this thesis whilst giving an intuitive derivation.
...
\[
K_{ij} = k(x_i, x_j) = x_i x_j + \delta_{x_i=x_j} \sigma_\varepsilon^2 \label{eq:intro:lin_noise_k}
\]
...
Finally, in chapter~\ref{ch:description} we demonstrate that by carefully choosing which kernels we compose with each other we can always automatically produce natural language descriptions of the probability distributions they encode for.
