...
We take a probabilistic and Bayesian approach throughout  which is the natural extension of drawing logical inferences from certain information~\citep[e.g.][]{Edwin2003-jh}.
To perform probabilistic inference one must be able to express all of one's prior information about some task probabilistically.
In chapters~\ref{ch:networks}~and~\ref{ch:arrays} we demonstrate how to construct probability distribution that respect certain symmetry conditions.
...
\end{enumerate}
The calculus that one can derive from these axioms alone is isomorphic to probability theory; plausabilities satisfy the sum and product rules that are traditionally stated as the axioms of probability theory\fTBD{cite e.g. Kolmogorov}.
