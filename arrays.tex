...
%Another example, movie ratings.
%Order of users and order of movies unimportant so we appeal to Aldous--Hoover after first mentioning that our data is encoded in an array.
%Cite some papers and mention that PMF (contrast with non-probabilistic SVD version) corresponds to this parameter space.

%Finally, a database may consist of many tables recording the value of many relations between different sets of objects \eg users, movies, actors, genres.
%Exchangeability of objects means an interesting coupled exchangeability of the tables recording the data (give an example).
%We extend the representation theorems to deal with this (perhaps give sneak preview of the result here).
...
Fix a finite set $\Types$ of \defn{types}.  By a \defn{signature} we mean a finite sequence $s \in \Types^n$ of types.
By a \defn{relation $r$ of signature $s \in \Types^n$ with values in a space $\Space$} we mean 
%a $n$-dimensional array of values of $\Space$.  More concretely, $r$ 
a function from $\Nats^n$ to $\Space$.
\end{definition}
...
By a \defn{database} we mean a collection of $R$ relations $r_1,\dotsc,r_R$ of signature $s_1,\dotsc,s_R$, respectively, taking values in $\Space_1,\dotsc,\Space_R$, respectively.
\end{definition}
