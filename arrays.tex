...
\begin{prop}[$\pi$-exchangeability representation theorem]
  ...
  uniformly to 1 as $n \to \infty$ for fixed $m$ where $\chi_m$ is the array subset operation such that $\chi_m X \defas \{X_{\bi} : \bi \in \{1,\dots,m\}^d\}$.
\end{prop}
...
Finally, let $\rho_j : \Nats^d \to \Nats^{d_j}$ be the projection that keeps the first $m_{1}^{s_j}$ dimensions, ignores the next $m_1 - m_{1}^{s_j}$, keeps the next $m_{2}^{s_j}$ et cetera.
...
\begin{thm}[Simple exchangeable database representation theorem]
  ...
  $\chi_m (X^{r_j,n})_{j=1}^R$ and $\chi_m (X^{r_j})_{j=1}^R)$ are mutually ansolutely continuous for all $m,n \in \Nats$ and the associated Radon--Nikodym denrivatives converge uniformly to 1 as $n \to \infty$ for fixed $m$.
\end{thm}
...
\begin{proof}
  ...
  \]
  By definition of an exchangeable database, this array is $\pi$ exchangeable with $\pi$ defined in the preceeding text.
  By proposition~\ref{thm:piex} there existss a sequence of measurable functions $(F^n)$ and collection of independent uniform random variables $U$ and $U^t_i$ such that $\chi_m R_{\bi}$ and $\chi_m F^n(U, U^{\pi_1}_{\bi_1}, \dots, U^{\pi_d}_{\bi_d})$ are mutually absolutely continuous and the associated Radon--Nikodym derivates converge uniformly to 1 as $n \to \infty$ for all $m$.
  TO BE CONTINUED
\end{proof}
...
\subsection{Simple databases: a conjecture (for the moment)}
...
\begin{conj}[Simple exchangeable database representation theorem]
  \label{thm:simple-database}
  Let $(X^{r_j})_{j=1}^R$ be an exchangeable database.
  Then there exist some simple exchangeable databases $(X^{r_j,1})_{j=1}^R$ , $(X^{r_j,2})_{j=1}^R$ , \dots such that
...
Unfortunately, we have not yet proven this result.
...
\TBD{
If a proof fails to appear, we can always prove the result with $m \leq M$ for arbitrary finite $M$.
A cop out but at least it gives more evidence as to why this conjecture might be true.
}

\section{A generic statistical model template}

In analogy to the work of \cite{Hoff2007-ja, Roy2009-ge, Lloyd2012-sb} on exchangeable arrays, conjecture\fTBD{I'm holding out for a proof, I will restructure if one is not forthcoming}~\ref{thm:as-database} naturally inspires a generic generative model of exchangeable databases.
