...
\begin{proof}
\fTBD{Might need to revert to super array proof}
Collections of arrays with this functional form are trivially exchangeable databases (see the corollaries below if the notation is confusing), so we include only the proof that exchangeable databases have this functional form\fTBD{Explain why it is trivial}.
...
Trivially\fTBD{Explain why it is trivial}, we can replace the $\pi_{s_j}$ in the previous expression with $\pi$ alone.
...
\end{equation} 
where $h$ is a bimeasurable function\fTBD{Careful: potentially different dependence structures across arrays for uniforms of different indexing structure}.
