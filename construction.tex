...
We focus on Gaussian process regression, where the kernel specifies a covariance function, because the Bayesian framework provides natural methods by which to select between models (see section \TBD{reference intro}).
...
This is made more precise is section \TBD{refer to future chapter}.
...
In section \TBD{forward reference} we introduce a modified version of the periodic kernel for which $\cos(x-x')$ is a limiting form.
...
\input{\constructionfigsdir/tables/mauna_growth.tex}
\input{\constructionfigsdir/tables/mauna_decomp.tex}
...
%
\input{\constructionfigsdir/tables/solar_decomp.tex}
%
The posterior and residuals of the learned kernel are shown in figure \ref{fig:solar_decomp}.
%
\input{\constructionfigsdir/tables/airline_decomp.tex}
%
...
\input{\constructionfigsdir/tables/regression_results_combined.tex}
...
In addition to the qualitative evaluations in sections \TBD{fix reference}, we investigated quantitatively how our method performs on both extrapolation and interpolation tasks.

\subsection{Extrapolation}

\input{\constructionfigsdir/tables/extrapolation.tex}
